...
function UnicodeConverter() {
    ...
    const convert = () => {
        try {
            const result = mode === 'toUnicode' ? textToUnicode(input) : unicodeToText(input);
            setOutput(result);
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-8">
            <div className="mb-8">
                <h1 className="text-4xl font-bold mb-2">Unicode Converter</h1>
            ...
            <div className="flex justify-center mb-6">
                <div className="inline-flex rounded-lg border border-gray-300 dark:border-gray-600">
                    <button onClick={() => setMode('toUnicode')} className={`px-6 py-2 font-medium rounded-l-lg ${mode === 'toUnicode' ? 'bg-primary-500 text-white' : 'bg-white dark:bg-gray-800'}`}>
                        Text → Unicode
                    </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-4">
                    ...
                    <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder={mode === 'toUnicode' ? 'Hello World' : '\\u0048\\u0065\\u006C\\u006C\\u006F'} className="input-field min-h-[300px] font-mono text-sm" />
